...
class AuthenticatedFlightApiTest(TestCase):
    ...
    def setUp(self):
        ...
        self.second_destination = Airport.objects.create(
            name="poltava_airport",
            closest_big_city="Poltava"
        )
    ...
    def test_filter_flight_by_city_from(self):
        ...
        flights = Flight.objects.filter(route__source__closest_big_city__icontains="dnipro")
        serializer = FlightListSerializer(flights, many=True)
        self.assertEqual(response.data["results"], serializer.data)
    ...
    def test_filter_flight_by_city_to(self):
        ...
        flights = Flight.objects.filter(route__destination__closest_big_city__icontains="lviv")
        serializer = FlightListSerializer(flights, many=True)
        self.assertEqual(response.data["results"], serializer.data)
